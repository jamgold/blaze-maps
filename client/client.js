...
function observerMarkers() {
  if (!markersObservation) {
    markersObservation = Markers.find().observeChanges({
      changed: function(id, fields) {
        console.log('changed marker', fields);
        var marker = map_canvas.markers[id];
        if (marker != undefined) { // && id != changingMarker) {
          console.log("changed mongo id " + id);
          // for (k in fields)
          ...
          {
            var loc = new google.maps.LatLng(R2D(fields.location.coordinates[1]), R2D(fields.location.coordinates[0]));
      ...
      removed: function(id, oldMarker) {
        if (map_canvas.markers[id] != undefined) {
          var marker = map_canvas.markers[id];
          map_canvas.mc.removeMarker(marker);
          // marker.setMap(null);
          delete(map_canvas.markers[id]);
...
Template.map.onRendered(function() {
  ...
  map_canvas.mc = new MarkerClusterer(map_canvas.map, [], {
    gridSize: 50,
    maxZoom: 15
  });
...
Template.map.events({
  ...
  'change input.marker_title': function(event, template) {
    ...
    //
    // this updates in DB but not the marker
    //
    Markers.update({
      _id: event.target.id
    }, {
